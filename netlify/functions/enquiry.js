// Netlify serverless function - handles product enquiry form to Notion.
...
function getNotionParent() {
  if (process.env.NOTION_ENQUIRY_DB_ID) return { database_id: process.env.NOTION_ENQUIRY_DB_ID };
...
exports.handler = async function (event) {
  ...
  if (!NOTION_TOKEN || !NOTION_PARENT) {
    console.error('Enquiry form is missing required Notion environment variables.');
  ...
  const altEmail = sanitize(data.altEmail || '', LIMITS.email).toLowerCase();
  ...
  const product = sanitize(data.product || '', LIMITS.product);
  const subject = sanitize(data.subject || '', LIMITS.subject);
  const quantity = sanitize(data.quantity || '', LIMITS.quantity);
  const message = sanitize(data.message, LIMITS.message);
  const sourcePage = sanitize(data.sourcePage || '/enquiry.html', LIMITS.sourcePage);

  if (!name || !email || !phone || !country || !product || !subject || !message) {
    return json(400, headers, { error: 'Missing required fields' });
  }

  if (!isValidEmail(email) || (altEmail && !isValidEmail(altEmail))) {
    return json(400, headers, { error: 'Invalid email address' });
  }

  const interest = quantity ? `${product} | Quantity: ${quantity}` : product;
  const properties = {
  ...
  if (altEmail) properties['Alt Email'] = { email: altEmail };
  if (subject) properties.Subject = richText(subject);
  if (quantity) properties.Quantity = richText(quantity);
  ...
  try {
    ...
    if (!res.ok) {
      console.error('Notion API error while saving enquiry form:', res.status);
      return json(502, headers, { error: 'Failed to save enquiry' });
    }

    return json(200, headers, { status: 'success' });
  } catch (err) {
    console.error('Enquiry form network error:', err.message);
